...
void do_donate( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg1 );
     location = get_room_index( hometown_table[ch->hometown].donation);
    if ( arg1[0] == '\0' )
    {
    ...
    act( "$p glows {Mpurple{x, then disappears from $n's inventory.", ch, obj, NULL, TO_ROOM );
    act( "$p glows {Mpurple{x, then disappears..", ch, obj, NULL, TO_CHAR );
...
void do_restring( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->qps < 6)
    {
	send_to_char( "Restrings cost 6 quest points, you do not have enough.\n\r", ch);
	return;
    }
...
void do_eat( CHAR_DATA *ch, char *argument )
{
    ...
    case ITEM_CAN_EAT:
         ...
         value = obj->value[0];
           ...
           if ( obj->value[0] != 0 )
             {
                 if ( IS_SET(obj->value[1], QUEST_POINT))
                    {
                      ch->qps += value;
                      break;
                     }
                 if ( IS_SET(obj->value[1], TRAIN_POINT))
                    {
                       ch->train += value;
                       break;
                    }
                 if ( IS_SET(obj->value[1], PRACTICE_POINT))
                    {
                       ch->practice += value;
                       break;
                    }
/*
                 if ( IS_SET(obj->value[1], NULL))
                    {
                      stc("ERROR, this was an empty item\n\r Please see an immortal\n\r",ch);
                      break;
                    }
*/ 
          }
...
if ( CAN_WEAR( obj, ITEM_WEAR_EAR ) )
	...
	send_to_char( "You already wear two ears.\n\r", ch );
	return;
    }
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	cost = get_cost( keeper, obj, TRUE );
        quest_cost = get_cost( keeper, obj, TRUE );
	...
	if (number > 1)
	{
        ...
        else if (IS_SET(ch->in_room->room_flags,  ROOM_QUEST)) 
        {

              if (ch->qps <= quest_cost)
                  {
                   stc("Sorry but you don't have enough for that.\n\r",ch);
                    return;
                   }
              else

               {
                  ...
                  sprintf(buf, "You buy $p for %d quest points.",quest_cost);
                  act( buf, ch, obj, NULL, TO_CHAR);
                  ch->qps -= cost;
...
void do_lore( CHAR_DATA *ch, char *argument )
{
  ...
  if (number_percent() < get_skill(ch,gsn_lore) &&
    ...
    switch ( obj->item_type )
      ...
      case ITEM_POTION:
      case ITEM_PILL:
	send_to_char( "Some level spells of:", ch );
	...
	break;
      ...
      case ITEM_WAND: 
      case ITEM_STAFF: 
	send_to_char( "Has some charges of some level", ch );
