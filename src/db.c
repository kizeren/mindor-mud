...
extern void spells_html_update( void );
...
void boot_db( void )
{
     ...
     * Loading classes
     */
     log_string( "Loading Classes Files." );
     load_classes();
    /*
     *Loading  hometowns
     */
     log_string( "Loading Hometown Files.");
     load_hometown();
...
/* OLC */     else if ( !str_cmp( word, "AREADATA" ) ) new_load_area(fpArea);
    ...
    log_string( "Updating Html Spells." );
         spells_html_update( );
...
#ifdef WIN32	
	pMem = (void*) (((char*)pMem) - sizeof (*magic));	// mkw:new idea
#else
    pMem -= sizeof(*magic);
#endif
    magic = (int *) pMem;
...
void load_hometown( void )
{
	...
	{
	case 0: 
	    sprintf( buf, "%snomad.home", HOME_DIR );
	    log_string( "File Nomad.home" ); 
	    break; 
	case 1:
	    sprintf( buf, "%smidgaard.home", HOME_DIR );
	    log_string( "File Midgaard.home" );
	    break;
	case 2:
	    sprintf( buf, "%snornir.home", HOME_DIR );
	    log_string( "File Nornir.home" );
	    break;
	...
	{
	    log_string( "Error opening hometown file." );
	    exit(1);
	}
	...
	for ( ;; )
	{
	        ...
	        for ( ;; )
		{
			...
			hometown_table[hometown].name = fread_string( fp );
		    else if ( !str_cmp( field, "WhoN" ) )
                       hometown_table[hometown].who_name = fread_string( fp );
		     else if( !str_cmp( field, "Recall" ) )
			hometown_table[hometown].recall = fread_number( fp );
		    else if ( !str_cmp( field, "School" ) )
			hometown_table[hometown].school = fread_number( fp );
		    else if ( !str_cmp( field, "Death" ) )
			hometown_table[hometown].death = fread_number( fp );
		    else if ( !str_cmp( field, "Donation" ) )
			hometown_table[hometown].donation = fread_number( fp );
