...
int		    ftp_control = -1;	/* Control socket for mudFTP
					 * (-1 for inactive)
					 */
/* Type of incoming connection: normal or mudFTP? */  
...
bool	read_from_descriptor	args( ( DESCRIPTOR_DATA *d ) );
bool	write_to_descriptor	args( ( int desc, char *txt, int length ) );
void    greet_ftp (DESCRIPTOR_DATA *d);
#endif
...
int main( int argc, char **argv )
{
	...
	*/
    ...
    wwwcontrol = init_socket( port + 6 );
/* Bind to the ftp port, portnumber is mainport+6 */
    ftp_control = init_socket (wwwport);
    boot_db( );
    sprintf( log_buf, "ROT is ready to rock on ports %d and %d, mudFTP port: %d", port, wwwport, port+6 );
    log_string( log_buf );
...
/*
		    nanny( d, d->incomm );
                break;
  */
  /* slight hack here so we can snarf all mudftp data */
                    while (d->incomm[0])
                    {
                        nanny (d, d->incomm);
...
void bust_a_prompt( CHAR_DATA *ch )
{
    ...
    bool found;
    bool round;
    const char *dir_name[] = {"N","E","S","W","U","D", "{DNE{x", "{DSE{x", 
                              "{DSW{x", "{DNW", "IN", "OUT"};
...
/* RT code for breaking link */
	...
	write_to_buffer( d, "\n\r", 2 );
        ...
        write_to_buffer( d, "1 None\n\r", 0);
        write_to_buffer( d, "  You will not have anything to do with the cities.\n\r",0);
        write_to_buffer( d, "2 Midggard\n\r", 0);
        write_to_buffer( d, "  Newbie friendly hometown.\n\r",0);
        write_to_buffer( d, "3 Nornir\n\r", 0);
        write_to_buffer( d, "  Newbie friendly, but without the 'goodies' of Midgaard.\n\r",0);
        write_to_buffer( d, "\n\r\n\rIf you are still unsure, log in and ask someone for help.\n\r",0);
	...
	switch ( argument[0] )
	{
	case '1': ch->hometown = 0;    
			    break;
	case '2': ch->hometown = 1;
			    break;
	case '3': ch->hometown = 2;
			    break;
	default:
	    write_to_buffer( d, "That's not a valid hometown\n\r", 0 );
        write_to_buffer( d, "Select a Hometown:\n\r", 0 );
        write_to_buffer( d, "1 None\n\r", 0);
        write_to_buffer( d, "  You will not have anything to do with the cities.\n\r",0);
        write_to_buffer( d, "2 Midggard\n\r", 0);
        write_to_buffer( d, "  Newbie friendly hometown.",0);
        write_to_buffer( d, "3 Nornir\n\r", 0);
        write_to_buffer( d, "  Newbie friendly, but without the 'goodies' of Midgaard.\n\r",0);
	    return;
	}
...
case CON_DEFAULT_CHOICE:
	...
	write_to_buffer( d, 
    "\n\rWelcome to Mindor.  Please feed the Gods.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	...
	reset_char(ch);
        ...
        sprintf( buf, "You have been taxed %d platinum for city funding.\n\r",
            tax );
        stc(buf,ch);
        ch->platinum -= tax;
        }
...
/* add this while initializing all the racial stuff for new characters */
	    ...
	    obj_to_char(create_object(get_obj_index(OBJ_VNUM_EMAP),0),ch);
  	    ...
  	    char_to_room( ch, get_room_index( hometown_table[ch->hometown].recall ));
...
/*	act( "$n has entered {WMindor{x.", ch, NULL, NULL, TO_ROOM );
	do_look( ch, "auto" );*/

         if (IS_IMMORTAL(ch) )
          {
	wiznet("$N has entered {WMindor{x.",ch,NULL,
		WIZ_LOGINS,WIZ_SITES,get_trust(ch));
        /* do_look( ch, "auto" ); */
        }
        else
        {
        do_announce( ch, "has entered {WMindor{x." );
        do_look( ch, "auto" );
        wiznet("$N has entered {WMindor{x.",ch,NULL,
                WIZ_LOGINS,WIZ_SITES,get_trust(ch));
	...
	if (ch->pet != NULL)
	{
	    ...
	    act("$n has entered {WMindor{x.",ch->pet,NULL,NULL,TO_ROOM);
	}
               who_html_update();
	...
	||    to->position < min_pos )
		...
		case '$':
			...
			switch ( *str )
			{
				...
				else if (ch->hometown == 1 )
				{
				    i = "Muaso";
				}
                                else if (ch->hometown == 0 )
                                {
                                    i = "wood sprites";
                                }
				break;
