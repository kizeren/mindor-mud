...
void spell_create_feast( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *mushroom;
    OBJ_DATA *orange;
    OBJ_DATA *apple;
    ...
    apple->value[0] = level / 2;
    apple->value[1] = level;
    orange = create_object( get_obj_index( OBJ_VNUM_ORANGE ), 0 );
    orange->value[0] = level / 2;
    orange->value[1] = level;
    bananna = create_object( get_obj_index( OBJ_VNUM_BANANNA ), 0 );
    bananna->value[0] = level / 2;
    bananna->value[1] = level;
    mushroom = create_object( get_obj_index( OBJ_VNUM_MUSHROOM ), 0 );
    mushroom->value[0] = level / 2;
    mushroom->value[1] = level;

    obj_to_room( spring, ch->in_room );
    obj_to_room( bananna, ch->in_room );
    obj_to_room( orange, ch->in_room );
    obj_to_room( mushroom, ch->in_room );
    ...
    act( "$p suddenly appears.", ch, mushroom, NULL, TO_ROOM );
    act( "$p suddenly appears.", ch, mushroom, NULL, TO_CHAR );
    act( "$p suddenly appears.", ch, apple, NULL, TO_ROOM );
    act( "$p suddenly appears.", ch, apple, NULL, TO_CHAR );
    act( "$p suddenly appears.", ch, bananna, NULL, TO_ROOM );
    act( "$p suddenly appears.", ch, bananna, NULL, TO_CHAR );
    act( "$p suddenly appears.", ch, orange, NULL, TO_ROOM );
    act( "$p suddenly appears.", ch, orange, NULL, TO_CHAR );
    return;
}
...
void spell_gate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    ||   IS_SET(victim->in_room->room_flags, ROOM_NOTPORT)
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
	send_to_char( "The spell fizzes and nothing happens.\n\r", ch );
	return;
    }
