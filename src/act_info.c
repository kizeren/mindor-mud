...
char *	const	where_name	[] =
{
    ...
    "{G<{Cworn on face{G>{x      ",
    "{G<{Cworn around ankle{G>{x ",
    "{G<{Cworn around ankle{G>{x ",
    "{G<{Cworn around ear{G>{x   ",
    "{G<{Cworn around ear{G>{x   "
};
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if ( victim->on_quest)			    strcat( buf, "({GQuest{x)");
       ...
       if (IS_NPC(victim) && (IS_SET(victim->act2, ACT2_QUESTMOB)))
    {
         if (victim->pIndexData->vnum == ch->questmob)
           {
              strcat(buf, "{G[{YQuest Target{G]{x");
           }
         else 
           {
              strcat(buf, "{G[{YQuest Mob{G]{x");
           }
    }
...
void do_mudftp(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->comm, COMM_MUDFTP))
    {
      send_to_char("You will no longer use mudFTP.\n\r",ch);
      REMOVE_BIT(ch->comm, COMM_MUDFTP);
    }
    else
    {
      send_to_char("You will use mudFTP for edits..\n\r",ch);
      SET_BIT(ch->comm,COMM_MUDFTP);
    }
 }
...
void do_prompt(CHAR_DATA *ch, char *argument)
{
   ...
   if( !strcmp( argument, "builder" ) )
      strcpy( buf, "{C[{G%z, %r -> %o{C] {R[{M%O -> %R{R]{x");
 
   else   if( !strcmp( argument, "all" ) )
      strcpy( buf, "{W<{r%h{W/{R%Hhp {m%m{W/{M%Mm {g%v{W/{G%Vmv{W %e>{x ");
   else
   {
...
door = 8;
    else if ( !str_cmp( arg1, "ne" ) || !str_cmp( arg1, "northwest"  ) ) 
door = 9;
    else if ( !str_cmp( arg1, "in" ) || !str_cmp( arg1, "inside"  ) ) 
door = 10;
    else if ( !str_cmp( arg1, "out" ) || !str_cmp( arg1, "outside"  ) ) 
door = 11;
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	add_buf(output,buf);
    sprintf( buf,
	"{R ________________________________________/{x%s%s{x\n\r",
	
	ch->name,ch->pcdata->last_name);
    add_buf(output,buf);
    sprintf( buf, "{R| {GPretitle: %s {GTitle: %s{x\n\r",ch->pcdata->pretitle,
	IS_NPC(ch) ? ", the mobile." : ch->pcdata->title);
    add_buf(output,buf);
    sprintf( buf, "{R| You are a %s of %s\n\r", 
           city_rank_table[ch->pcdata->city_ranks].rank, 
           hometown_table[ch->hometown].who_name);
    add_buf(output,buf);
    sprintf( buf,
    ...
    if (ch->pcdata->remorts)
    {
        if (ch->pcdata->remorts == 1)
            sprintf( buf, "{R| {xYou have {M%d{x rebirth.\n\r", ch->pcdata->remorts );
        else
            sprintf( buf, "{R| {XYou have {M%d{x rebirths.\n\r",ch->pcdata->remorts );
        add_buf(output,buf);
    }
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char(buf,ch);
    sprintf(buf,"Mindor started up at %s\n\rThe system time is %s\n\r",
	str_boot_time,
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg,wch->name))
	{
		...
		wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name
					: "     ",
		class,
             wch->sex == 0 ? "{6N{x" : wch->sex == 1 ? "{4M{x" : "{mF{x",
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    output = new_buf();
    outputimm = new_buf();
    add_buf(outputimm,"\n\r                   The Gods and Mortals of Mindor.\n");
    add_buf(outputimm,"{r<>{D---------------------------------------------------------------------{r<>\n");
    add_buf(outputimm,"{D| {WLVL  Race  Class PK   Name & Title                                    {D|\n");
    add_buf(outputimm,"{r<>{D---------------------------------------------------------------------{r<>{x\n");
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        CHAR_DATA *wch;
	char class[MAX_STRING_LENGTH];
 ...
 /*
	 ...
	 */
        if (!wch->on_quest)
        {
            ...
            sprintf(questdat, "{W[{CQ{cu{Ce{cs{Ct{ci{Cn{cg{W]{x ");
        }
        if (!is_clan(wch)) 
        { 
	    sprintf(clandat, "%s", clan_table[wch->clan].who_name);
	...
	{
	    sprintf( buf, "{D| %-19s {W{D |{x %s%s%s%s %s%s%s%s%s%s\n\r",
		
		wch->pcdata->who_descr,
                wch->pcdata->pretitle,
		wch->short_descr, wch->pcdata->last_name,
		IS_NPC(wch) ? "" : wch->pcdata->title,questdat,
		clandat,
		((wch->ghost_level >= LEVEL_HERO)&&(ch->level >= wch->level)) ? "(Ghost) ": "",
		wch->incog_level >= LEVEL_HERO ? "(Incog) " : "",
		wch->invis_level >= LEVEL_HERO ? "(Wizi) " : "",
	...
	{
	    sprintf( buf, "{D| {y%s  {D%-6s{x %s  {R%-1s{D | {x %s%s%s%s %s%s%s%s%s%s\n\r",
		buf2,
               wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
		class,
            IS_SET(wch->act, PLR_PROKILLER) ? "K" : "*",
                wch->pcdata->pretitle,
		wch->short_descr, wch->pcdata->last_name,
		IS_NPC(wch) ? "" : wch->pcdata->title,questdat,
		clandat,
		...
		wch->incog_level >= LEVEL_HERO ? "(Incog) " : "",
		wch->invis_level >= LEVEL_HERO ? "(Wizi) " : "",
		IS_SET(wch->comm, COMM_AFK) ? "{G[{yA{YF{yK{G]{x " : "");
    ...
    sprintf(buf2,"{r<>{D---------------------------------------------------------------------{r<>{x\n");
    add_buf(output,buf2);

    max_on = UMAX(count,max_on);
    if (doublexp)
    {
    sprintf(buf2,"{D| {GThe Extra Experience flag is on.                                      {D|\n{x");
    add_buf(output,buf2);
    }
    if (global_quest)
    {
    sprintf( buf2,"{D| {GThe global quest flag is on.                                         {D|{x\n");
    add_buf(output,buf2);
    }
    if (nMatch != count)
    {
	sprintf( buf2, "{BMatches found: {W%d{x\n", nMatch );
	add_buf(output,buf2);
    } else {
    sprintf( buf2,"{D| {BPlayers found: {W%d  {BMost on today: {W%d                                    {D|{x\n", count, max_on );
    add_buf(output,buf2);
    sprintf( buf2,"{r<>{D---------------------------------------------------------------------{r<>{x\n\r");
    add_buf(output,buf2);
    }
...
char buf[MSL];
    if (arg1[0] == '\0') {
        send_to_char("Enter the name of the spell or class.\n\r",ch);
        return;
    }
        if (((skill = skill_lookup(arg1)) == -1) && (cl = class_lookup(arg1)) == -1) {
        send_to_char("That is not a valid class or skill.\n\r",ch);
        return;
    }
    ...
    if (((skill = skill_lookup(arg1)) == -1) && (cl = class_lookup(arg1)) != -1) {
      ...
      if (class_name == MAX_CLASS) {
        ...
        return;
          ...
          sprintf(buf,"{CAviable skill/spells for the class{c: {W%s{x\n\r", class_table[cl].name);
          send_to_char(buf,ch);
          send_to_char("{cSpells{W:{x\n\r",ch);
          ...
          send_to_char("\n\r",ch); col = 0;
          sprintf(buf, "{CTotal spells found{g: {W%d{x\n\r",spCount);
          send_to_char(buf,ch);
          send_to_char( "{cSkills{W:{x\n\r",ch);
          for (level = 1 ;level <= 101;level++) {
              for (sn = 0; sn < MAX_SKILL; sn++) {
                  if (skill_table[sn].skill_level[cl] > 100)
                      continue;
          ...
          send_to_char("\n\r",ch);
          sprintf(buf, "{cBase Group{W: {W%s{x\n\r",class_table[cl].base_group);
          send_to_char(buf,ch);
          gn = 0; col = 0; level = 1;
    ...
    if (((skill = skill_lookup(arg1)) != -1) && (cl = class_lookup(arg1)) == -1) {
        ...
        if (skill_table[skill].skill_level[class] > 100)
            continue;
        sprintf(buf,"{G%-11s {g%5d %4d %9d %8d %6d\n\r",
...
void do_announce( CHAR_DATA *ch, char *argument, char *message )
{
    char buf[MAX_STRING_LENGTH];
    DESCRIPTOR_DATA *d;
      ...
      sprintf( buf, "{D[{RINFO{D] $n {L%s{x", argument );
    act_new("{D[{RINFO{D] $n {L$t{x",ch,argument,NULL,TO_CHAR,POS_DEAD);
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        if ( d->connected == CON_PLAYING &&
             d->character )
        {   
            act_new("{D[{RINFO{D] $n {L$t{x",ch,argument,d->character,TO_VICT,POS_DEAD);
         } 
    }
    return;
}
...
argument = one_argument(argument, arg);
    ...
    ch->platinum += amnt;
    ch->pcdata->bank -= amnt;
    sprintf( buf, "{R%s{G, your account now contains: {W%d{G coins, after withdrawing {W%d{G coins.\n\r", ch->name, ch->pcdata->bank, amnt );
    send_to_char( buf, ch );
    return;
}
