...
bool is_same_hometown(CHAR_DATA *ch, CHAR_DATA *victim)
{
   if (!hometown_table[ch->hometown].name)
       return FALSE;
   else
       return (ch->hometown == victim->hometown);
}
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    case ITEM_CAN_EAT:          return "can_eat";
    }
...
char *act_bit_name( int act_flags )
{
	...
	if (act_flags & ACT_IS_HEALER	) strcat(buf, " healer");
        if (act_flags & ACT_IS_SPELLUP  ) strcat(buf, " spellup");
...
char *act2_bit_name( int act2_flags )
{
    ...
    { 
 	strcat(buf," npc");
	if (act2_flags & ACT2_NONE) strcat(buf," none");
        if (act2_flags & ACT2_QUESTMOB  ) strcat(buf, "QUEST");
    }
    else
    {
	strcat(buf," player");
	if (act2_flags & PLR2_NONE	) strcat(buf, " none");
    }
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NOSOCIAL      ) strcat(buf, " nosocial");
    if (comm_flags & COMM_NOCOMMANDS    ) strcat(buf, "nocommands");
