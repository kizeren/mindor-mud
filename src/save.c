...
DECLARE_DO_FUN(update_statlist);
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
 ...
 fprintf( fp, "Bank  %d\n", ch->pcdata->bank     );
    ...
    if (ch->clead)
        ...
        fprintf( fp, "ShopM %d\n",  ch->pcdata->shopmob_vnum );
    fprintf( fp, "ShpV %d\n",   ch->pcdata->shop_vnum );
    fprintf(fp, "Hmtown %d\n", ch->hometown);
    fprintf(fp, "Crank %d\n", ch->pcdata->city_ranks);
    if (ch->pcdata->spouse)
    {
         fprintf(fp,"Spou %s~\n",ch->pcdata->spouse);
    }
    ...
    fprintf( fp, "Nta  %ld %ld %ld\n",
    ch->pcdata->last_personal,ch->pcdata->last_city,ch->pcdata->last_family);
    fprintf( fp, "Scro %d\n",   ch->lines       );
    fprintf( fp, "Room %d\n",
    ...
    if (ch->pcdata->bamfout[0] != '\0')
        fprintf( fp, "Bout %s~\n",  ch->pcdata->bamfout);
	if (ch->pcdata->wiziname[0] != '\0')
		fprintf (fp, "WNc %s~\n", ch->pcdata->wiziname);
	if (ch->pcdata->storedwiziname[0] != '\0') {
		fprintf (fp, "WNs %s~\n", ch->pcdata->storedwiziname);
		fprintf (fp, "WNt %d\n", ch->pcdata->wizinametoggle);
	}
...
void fwrite_obj( CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest )
{
     ...
     */
    if ( (ch->level < obj->level - 20 && obj->level > 19
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->pcdata->talk_color     = 0;
	ch->pcdata->storedwiziname = str_dup ("");
    ...
    ch->pcdata->remorts                 = 0;
    ch->pcdata->city_ranks               = 0;
   ch->pcdata->bounty                 = 0; 
   ch->hometown            = 0;

   ch->pcdata->last_personal = 0;
   ch->pcdata->last_family = 0;
   ch->pcdata->last_city    = 0;
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    case 'C':
        ...
        KEY( "Cowiz",   ch->color_wiz,      fread_number( fp ) );
            KEY( "Crank",       ch->pcdata->city_ranks,  fread_number( fp ) );
...
if (!str_cmp(word, "GStats"))
    ...
    if ( !str_cmp( word, "Lname" )  || !str_cmp( word, "Lnam"))
            {
                ch->pcdata->last_name = fread_string( fp );
                if (ch->pcdata->last_name[0] != '.' && ch->pcdata->last_name[0] != ','
                &&  ch->pcdata->last_name[0] != '!' && ch->pcdata->last_name[0] != '?')
                {
    ...
    case 'M':
        KEY( "Mar",     ch->pcdata->married,   fread_number( fp ) );
        break;
    ...
    case 'N':
        ...
        ch->pcdata->last_city           = fread_number(fp);
        ch->pcdata->last_family        = fread_number(fp);
        fMatch = TRUE;
        break;
        } 
        if (!str_cmp(word,"Not"))
   ...
   if ( !str_cmp( word, "Pretitle" )  || !str_cmp( word, "pretit"))
            {
    ...
    case 'R':
            KEY( "Remort",    ch->pcdata->remorts,  fread_number( fp ) );
        KEY( "Race",        ch->race,   
    ...
    case 'S':
	    KEY( "ShopM",        ch->pcdata->shopmob_vnum, fread_number( fp ) );
	    KEY( "ShpV",        ch->pcdata->shop_vnum,  fread_number( fp ) );
    ...
    case 'T':
        ...
        KEY ( "TCol", ch->pcdata->talk_color, fread_number ( fp ) );
    ...
    case 'W':
		...
		KEY ("WNc", ch->pcdata->wiziname, fread_string (fp));
		KEY ("WNs", ch->pcdata->storedwiziname, fread_string (fp));
		KEY ("WNt", ch->pcdata->wizinametoggle, fread_number (fp));
        break;
    }
