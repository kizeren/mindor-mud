...
*/
    ...
    case POS_DEAD:if 
        ((IS_NPC(victim)) && ( victim->die_descr != NULL && victim->die_descr[0] != '\0'))
    {
...
victim->pcdata->bounty, victim->short_descr);
    send_to_char(buf, ch);
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if ((IS_SET(victim->act2, ACT2_QUESTMOB)) && (victim->pIndexData->vnum != ch->questmob))
       {
           stc("Woops, that's someone's quest mob.\n\r",ch);
           return TRUE;
       }
    ...
    ||  IS_SET(victim->act,ACT_IS_PRIEST)
        ||  IS_SET(victim->act,ACT_IS_SPELLUP))
    {
    ...
    else
    {
        ...
        return TRUE;
            ...
            if (!IS_SET(ch->act, PLR_PROKILLER) && 
               (!IS_SET(ch->in_room->sector_type, SECT_ARENA)))
            {
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
    ...
    if ( ( location = get_room_index( hometown_table[ch->hometown].death ) ) == NULL )
    {
      stc( "Your Morgue could not be found!",ch);
      return;
    }
    ...
    else if (!IS_SET(ch->act, PLR_PROKILLER)) 
    {
...
void raw_kill( CHAR_DATA *victim, CHAR_DATA *killer )
{
    ...
    if (!IS_NPC(victim))
    {
	...
	else
        {
            ...
            killer->pcdata->gamestat[PK_KILLS]++;
        }
    }
    else
    {
        if (!IS_NPC(killer))
        {
            killer->pcdata->gamestat[MOB_KILLS]++;
        } 
	else
        {
            kill_table[URANGE(0, killer->level, MAX_LEVEL - 1)].kills++;
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( is_same_group( gch, ch ) )
        {
        members++;
        group_levels += IS_NPC(gch) ? gch->level / 1 : gch->level;
    }
    }
...
*/
  ...
  if (IS_SET(ch->act2, PLR2_QUESTOR) && IS_NPC(victim))
    {
       if (ch->questmob == victim->pIndexData->vnum && (IS_SET(victim->act2, ACT2_QUESTMOB)))
           {
              stc("You have almost completed your QUEST!\n\r",ch);
              stc("Return to the questmaster before your time runs out!\n\r",ch);
              ch->questmob = -1;
           }
    }
  ...
  if (doublexp)

       {
        ...
        sprintf (buf, "You receive %d extra experience points.\n\r", xp);
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
    ...
    else if ( dam <=  60 ) { vs = "{W*** {DD{WE{DM{WO{DL{WI{DS{WH {W***{x";
                 vp = "{W*** {DD{WE{DM{WO{DL{WI{DS{WH{DE{WS {W***{x";           }
    else if ( dam <=  75 ) { vs = "{W*** {YD{ME{YV{MA{YS{MT{YA{MT{YE {W***{x";
                 vp = "{W*** {YD{ME{YV{MA{YS{MT{YA{MT{YE{MS {W***{x";           }
    else if ( dam <= 100)  { vs = "{W=== {CO{cB{CL{cI{CT{cE{CR{cA{CT{cE {W==={x";
                 vp = "{W=== {CO{cB{CL{cI{CT{cE{CR{cA{CT{cE{CS {W==={x";        }
    else if ( dam <= 125)  { vs = "{W>>> {GA{BN{GN{BI{GH{BI{GL{BA{GT{BE {W<<<{x";
                 vp = "{W>>> {GA{BN{GN{BI{GH{BI{GL{BA{GT{BE{GS {W<<<{x";        }
    else if ( dam <= 150)  { vs = "{W<<< {ME{WR{MA{WD{MI{WC{MA{WT{ME {W>>>{x";
                 vp = "{W<<< {M{WE{MR{WA{MD{WI{MC{WA{MT{WE{MS {W>>>{x";         }
    else if ( dam <= 250)  { vs = "do {RU{WN{RS{WP{RE{WA{RK{WA{RB{WL{RE{x things to";
                 vp = "does {RU{WN{RS{WP{RE{WA{RK{WA{RB{WL{RE{x things to";     }
    else                   { vs = "do {YU{RN{YG{RO{YD{RL{YY{x things to";
                             vp = "does {YU{RN{YG{RO{YD{RL{YY{x things to";             }
    ...
    {
        sprintf( buf1, "{k$n %s $N%c {r<{R<{D%d{R>{r>{x", vp, punct, 
dam );
        sprintf( buf2, "{hYou %s $N%c {w<{W<{G%d{W>{w>{x", vs, punct, 
dam );
        sprintf( buf3, "{i$n %s you%c {r<{R<{D%d{R>{r>{x", vp, punct, 
dam );
    }
    }
    else
    ...
    else
    {
        ...
        {
            sprintf( buf1, "{k$n's %s %s $N%c {r>{R>{D%d{R<{r<{x",  attack, vp, punct,dam );
            sprintf( buf2, "{hYour %s %s $N%c {w>{W>{G%d{W<{w<{x",  attack, vp, punct,dam );
            sprintf( buf3, "{i$n's %s %s you%c {r>{R>{D%d{R<{r<{x", attack, vp, punct,dam );
        }
    }
    }
...
void do_feed( CHAR_DATA *ch, char *argument )
{
    ...
    dam=number_range( (((ch->level/2)+(victim->level/2))/2),
        (((ch->level/2)+(victim->level/2))/2)*2 );
...
void do_fish( CHAR_DATA *ch, char *argument )
{
    ...
    if ((chance = get_skill(ch,gsn_fish)) == 0)
    {
   ...
   if (!IS_SET(ch->in_room->sector_type, SECT_WATER_SWIM))
   {
    send_to_char("You must be near or in water to do this.\n\r",ch);
    return;
   }
   if (ch->move <= 20)
     {
     send_to_char("You need to rest up before fishing.",ch);
     return;
     }

   if (!IS_NPC(ch) 
   && ch->level < skill_table[gsn_fish].skill_level[ch->class])
   {
    ...
    send_to_char("You have caught a fish.x\n\r",ch);
    act("{k$n has caught a fish.{x",ch,NULL,NULL,TO_ROOM);
    ...
    send_to_char("{hYou missed the fish.{x\n\r",ch);
    check_improve(ch,gsn_fish,FALSE,2);
    }
