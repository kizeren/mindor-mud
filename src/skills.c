...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	act("$N helps you apply your hit points to training",
		ch,NULL,trainer,TO_CHAR);
        ch->max_hit -= 10;
	ch->train +=1 ;
	return;
	...
	wait_act(0,"$N trains you.",
	    ch,NULL,trainer,TO_CHAR);
        wait_act(1,"$N trains you.",
            ch,NULL,trainer,TO_CHAR);
        wait_act(2,"$N trains you.",
            ch,NULL,trainer,TO_CHAR);
        wait_act(3,"$N trains you.",
            ch,NULL,trainer,TO_CHAR);
        wait_act(4,"$N says, '{YYou should be at more ease with your skills now.{x'",
            ch,NULL,trainer,TO_CHAR);
	ch->train -= 2;
	ch->pcdata->points -= 1;
	...
	/* add the group */
	gn_add(ch,gn);
	wait_act(1,"$N trains you in the art of $t",
	    ch,group_table[gn].name,trainer,TO_CHAR);
        wait_act(2,"$N shakes $S head as $E shows you what you have done wrong.\n\r",
            ch,0,trainer,TO_CHAR);
        wait_act(3,"$N nods and says, '{YYou are now ready to practice your new skill.{x'\n\r",
            ch,0,trainer,TO_CHAR);
	...
	ch->pcdata->learned[sn] = 1;
       ...
       wait_act(2,"$N shrugs, as $S wonders what went wrong.",
            ch,0,trainer,TO_CHAR);
       wait_act(3,"$N nods, as $E thinks you are now ready.",
            ch,0,trainer,TO_CHAR);
