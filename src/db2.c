...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->race		 	= race_lookup(fread_string( fp ));
        ...
        pMobIndex->act2                 = fread_flag( fp ) | ACT2_IS_NPC
					| race_table[pMobIndex->race].act2;
...
void load_classes( void )
{
	...
	case 3:
	    ...
	    break;
	case 4:
	    sprintf( buf, "%sranger.class", CLASS_DIR );
	    log_string( "Loading Ranger." );
	    break;
	case 5:
	    sprintf( buf, "%sdruid.class", CLASS_DIR );
	    log_string( "Loading Druid." );
	    break;
	case 6:
	    sprintf( buf, "%svampire.class", CLASS_DIR );
	    log_string( "Loading Vampire." );
	    break;
	case 7:
	    sprintf( buf, "%snoble.class", CLASS_DIR );
	    log_string( "Loading Noble." );
	    break;
/*****************above this line for tier 1 **********************/
/*****************below for tier 2 ********************************/
	case 8:
	    sprintf( buf, "%swizard.class", CLASS_DIR );
	    log_string( "Loading Wizard." );
	    break;
	case 9:
	    sprintf( buf, "%spriest.class", CLASS_DIR );
	    log_string( "Loading Priest." );
	    break;
	case 10:
	    sprintf( buf, "%smercenary.class", CLASS_DIR );
	    log_string( "Loading Mercenary." );
	    break;
	case 11:
	    sprintf( buf, "%sgladiator.class", CLASS_DIR );
	    log_string( "Loading Gladiator." );
	    break;
	case 12:
	    sprintf( buf, "%sstrider.class", CLASS_DIR );
	    log_string( "Loading Paladin/Strider." );
	    break;
	case 13:
	    sprintf( buf, "%ssage.class", CLASS_DIR );
	    log_string( "Loading sage." );
	    break;
	case 14:
	    sprintf( buf, "%slich.class", CLASS_DIR );
	    log_string( "Loading lich." );
	    break;
	case 15:
	    sprintf( buf, "%sknight.class", CLASS_DIR );
	    log_string( "Loading Knight." );
	    break;
	}
